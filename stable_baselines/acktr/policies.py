...
class GaussianMlpPolicy(object):
    def __init__(self, ob_dim, ac_dim):
        ...
        ob_no = tf.compat.v1.placeholder(tf.float32, shape=[None, ob_dim * 2], name="ob")  # batch of observations
        oldac_na = tf.compat.v1.placeholder(tf.float32, shape=[None, ac_dim], name="ac")  # batch of actions previous actions
        # batch of actions previous action distributions
        oldac_dist = tf.compat.v1.placeholder(tf.float32, shape=[None, ac_dim * 2], name="oldac_dist")
        adv_n = tf.compat.v1.placeholder(tf.float32, shape=[None], name="adv")  # advantage function estimate
        wd_dict = {}
        ...
        # Variance on outputs
        self.logstd_1a = logstd_1a = tf.compat.v1.get_variable("logstd", [ac_dim], tf.float32, tf.compat.v1.zeros_initializer())
        logstd_1a = tf.expand_dims(logstd_1a, 0)
        std_1a = tf.exp(logstd_1a)
        std_na = tf.tile(std_1a, [tf.shape(input=mean_na)[0], 1])
        ac_dist = tf.concat([tf.reshape(mean_na, [-1, ac_dim]), tf.reshape(std_na, [-1, ac_dim])], 1)
        # This is the sampled action we'll perform.
        sampled_ac_na = tf.random.normal(tf.shape(input=ac_dist[:, ac_dim:])) * ac_dist[:, ac_dim:] + ac_dist[:, :ac_dim]
        logprobsampled_n = - tf.reduce_sum(input_tensor=tf.math.log(ac_dist[:, ac_dim:]), axis=1) - 0.5 * tf.math.log(
            2.0 * np.pi) * ac_dim - 0.5 * tf.reduce_sum(
            input_tensor=tf.square(ac_dist[:, :ac_dim] - sampled_ac_na) / (tf.square(ac_dist[:, ac_dim:])),
            axis=1)  # Logprob of sampled action
        logprob_n = - tf.reduce_sum(input_tensor=tf.math.log(ac_dist[:, ac_dim:]), axis=1) - 0.5 * tf.math.log(
            2.0 * np.pi) * ac_dim - 0.5 * tf.reduce_sum(
            input_tensor=tf.square(ac_dist[:, :ac_dim] - oldac_na) / (tf.square(ac_dist[:, ac_dim:])),
            axis=1)  # Logprob of previous actions under CURRENT policy (whereas oldlogprob_n is under OLD policy)
        kl_loss = tf.reduce_mean(input_tensor=kl_div(oldac_dist, ac_dist, ac_dim))
        ...
        surr = - tf.reduce_mean(input_tensor=adv_n * logprob_n)  # Loss function that we'll differentiate to get the policy gradient
        surr_sampled = - tf.reduce_mean(input_tensor=logprob_n)  # Sampled loss of the policy
