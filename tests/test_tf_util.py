...
def test_function():
    ...
    with tf.Graph().as_default():
        x_ph = tf.compat.v1.placeholder(tf.int32, (), name="x")
        y_ph = tf.compat.v1.placeholder(tf.int32, (), name="y")
        z_ph = 3 * x_ph + 2 * y_ph
...
def test_multikwargs():
    ...
    with tf.Graph().as_default():
        ...
        with tf.compat.v1.variable_scope("other"):
            x2_ph = tf.compat.v1.placeholder(tf.int32, (), name="x")
        z_ph = 3 * x_ph + 2 * x2_ph
...
def test_image_detection():
    ...
    for shape in (rgb, gray, rgbd):
        assert is_image(tf.compat.v1.placeholder(tf.uint8, shape=shape))

    for shape in (invalid_1, invalid_2):
        assert not is_image(tf.compat.v1.placeholder(tf.uint8, shape=shape))
